...
function pt(thetaDeg, r = R) { const a = thetaDeg * DEG; return [CX + r * Math.sin(a), CY - r * Math.cos(a)]; }
...
function drawStatic(dayFrac) {
  ...
  const pl = $("partLabels"); pl.innerHTML = "";
  const label = (txt, a, r) => {
    const [x, y] = pt(a, r);
    const t = document.createElementNS("http://www.w3.org/2000/svg", "text");
    t.setAttribute("x", x); t.setAttribute("y", y);
    t.setAttribute("text-anchor", "middle"); t.setAttribute("dominant-baseline", "middle");
    t.textContent = txt; pl.appendChild(t);
  };
...
function placeMarker(f, b, now) {
  ...
  const [x, y] = pt(theta);
  const m = $("marker"), g = $("glow");
  m.setAttribute("cx", x); m.setAttribute("cy", y);
  g.setAttribute("cx", x); g.setAttribute("cy", y);
  if (f.daytime) { m.setAttribute("fill", DAY);   m.setAttribute("r", 8); g.style.opacity = 0; }
  else           { m.setAttribute("fill", NIGHT); m.setAttribute("r", 7); g.style.opacity = 0; }
}

// ---------- App ----------
let LAT = 48.8566, LNG = 2.3522, LOCNAME = "Paris", LOCTZ = "Europe/Paris";
let currentFrac = null, animRAF = null, animating = false, animTarget = null, pendingAnimate = false;
...
function setDial(target, animate) {
  ...
  if (currentFrac === null) { drawStatic(target); currentFrac = target; return; }
  if (!animate) {
    if (animating && animTarget !== null && Math.abs(target - animTarget) < 0.01) return; // don't interrupt a tween to the same target
    if (animating) { if (animRAF) cancelAnimationFrame(animRAF); animating = false; }
    if (Math.abs(target - currentFrac) > 0.0005) { drawStatic(target); currentFrac = target; }
    return;
  }
  if (Math.abs(target - currentFrac) < 0.0015) return;
  if (animRAF) cancelAnimationFrame(animRAF);
  const from = currentFrac, to = target, t0 = performance.now(), dur = 520;
  const ease = x => 1 - Math.pow(1 - x, 3);
  animating = true; animTarget = to;
  const stepFn = (nowT) => {
    const p = Math.min(1, (nowT - t0) / dur);
    const frac = from + (to - from) * ease(p);
    drawStatic(frac); currentFrac = frac;
    if (p < 1) animRAF = requestAnimationFrame(stepFn);
    else { animating = false; currentFrac = to; }
  };
  ...
  if (LOCTZ) {
    try {
      return new Intl.DateTimeFormat("en-GB",
        { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: LOCTZ }).format(d);
...
function dateLabel(d) {
  ...
  if (LOCTZ) { try { return new Intl.DateTimeFormat("en-GB", { ...opt, timeZone: LOCTZ }).format(d); } catch (e) { /* fall through */ } }
...
function seasonDate(doy) { return new Date(Date.UTC(new Date().getFullYear(), 0, doy, 12, 0, 0)); }
function seasonLabel(d, long) { return new Intl.DateTimeFormat("en-GB", { day: "numeric", month: long ? "long" : "short", timeZone: "UTC" }).format(d); }
function latLabel(lat) { const a = Math.abs(Math.round(lat)); return a === 0 ? "0° (equator)" : a + "° " + (lat >= 0 ? "N" : "S"); }
function hm(ms) { return Math.floor(ms / 3600000) + " h " + pad(Math.round((ms % 3600000) / 60000)) + " m"; }
function liveCivil(now, f) {
  return 'Civil time <b>' + hhmm(now) + '</b> &nbsp;·&nbsp; sunrise <b>' + hhmm(f.sr) + '</b> &nbsp;·&nbsp; sunset <b>' + hhmm(f.ss) + '</b>';
}
...
function tick() {
  ...
  const now = S.now, f = frameFor(now, S.lat, S.lng);
  $("err").textContent = "";
  ...
  if (f.polar) {
    $("oHour").textContent = "--"; $("oUsh").textContent = "--"; $("oGar").textContent = "--";
    ...
    $("civilLine").innerHTML = explore.active
      ...
      : "";
    currentFrac = null; pendingAnimate = false;
    return;
  }
  const b = babylon(now, f);
  setDial(b.dayFrac, pendingAnimate); pendingAnimate = false;
  placeMarker(f, b, now);
...
function setLocation(lat, lng, name, tz) {
  LAT = lat; LNG = lng; LOCNAME = name; LOCTZ = tz || null;
  explore.active = false;
  const er = document.getElementById("exploreReset"); if (er) er.hidden = true;
  const ls = document.getElementById("latSlider"); if (ls) ls.value = Math.round(lat);
  const ss = document.getElementById("seasonSlider"); if (ss) ss.value = dayOfYear(new Date());
  updateExploreLabels();
  pendingAnimate = true;
  tick();
}
...
const search = $("citySearch"), listEl = $("cityList");
...
function filterCaps(q) {
  ...
  if (!nq) return CAPITALS.slice().sort((a, b) => a.n.localeCompare(b.n)).slice(0, 60);
  const starts = [], has = [];
  for (const c of CAPITALS) {
    const n = norm(c.n), cc = norm(c.c);
...
function renderList() {
  ...
  matches = filterCaps(q); active = -1;
  if (!matches.length) {
    listEl.innerHTML = '<li class="empty">No capital matches — try “Use my location” or coordinates.</li>';
  } else {
    listEl.innerHTML = matches.map((c, i) =>
      `<li role="option" data-i="${i}"><span class="nm">${hi(c.n, q)}</span><span class="cty">${hi(c.c, q)}</span></li>`).join("");
  }
  openList();
}
function openList() { listEl.hidden = false; search.setAttribute("aria-expanded", "true"); }
function closeList() { listEl.hidden = true; search.setAttribute("aria-expanded", "false"); active = -1; }
function choose(i) {
...
function setActive(i) {
  ...
  const el = items[active]; el.classList.add("active"); el.scrollIntoView({ block: "nearest" });
}

search.addEventListener("focus", renderList);
search.addEventListener("input", renderList);
search.addEventListener("keydown", e => {
  if (listEl.hidden && (e.key === "ArrowDown" || e.key === "ArrowUp")) { renderList(); return; }
...
$("useLoc").addEventListener("click", () => {
  if (!navigator.geolocation) { $("err").textContent = "Geolocation not available in this browser."; return; }
  $("err").textContent = "Locating…";
  const browserTz = Intl.DateTimeFormat().resolvedOptions().timeZone;
  navigator.geolocation.getCurrentPosition(
    p => setLocation(p.coords.latitude, p.coords.longitude, "My location", browserTz),
    e => { $("err").textContent = "Location denied — pick a city or enter coordinates."; },
    { timeout: 8000 }
  );
});
$("setManual").addEventListener("click", () => {
  ...
  } else { $("err").textContent = "Enter a valid latitude (-90..90) and longitude (-180..180)."; }
});

// ----- explore slider wiring -----
const seasonSlider = $("seasonSlider"), latSlider = $("latSlider"), exploreReset = $("exploreReset");
...
function highlightDetents() {
  ...
  document.querySelectorAll("#detents .detent").forEach(b => b.classList.toggle("on", +b.dataset.doy === v));
}
function updateExploreLabels() {
  const hit = DETENTS.find(x => x.doy === +seasonSlider.value);
  $("seasonVal").textContent = seasonLabel(seasonDate(+seasonSlider.value), false) + (hit ? " · " + hit.name : "");
...
function snapSeason() {
  ...
  for (const d of DETENTS) { const dist = Math.abs(v - d.doy); if (dist < bd) { bd = dist; best = d; } }
  if (best && bd <= SNAP) seasonSlider.value = best.doy;
}
seasonSlider.addEventListener("input", () => { snapSeason(); enterExplore(false); });
latSlider.addEventListener("input", () => enterExplore(false));
$("detents").addEventListener("click", e => {
  const b = e.target.closest(".detent"); if (!b) return;
